...
- **Architecture Decision Log** (`architecture-decisions.md`) — Owns the strategic reasoning: why LLM-as-feature-extractor, why no framework, why engineered weakness, why single pipeline. Read this for "why did we choose this approach?" context.
...
│   ├── po_matching.py         # Step 1: PO lookup            → docs/po-matching.md
│   ├── attribute_extraction.py # LLM extraction              → docs/llm-extraction.md
│   ├── classification.py      # Step 2: priority rules       → docs/classification.md
│   ├── treatment.py           # Step 3: prepaid/accrual      → docs/treatment.md
│   ├── approval.py            # Step 4: routing              → docs/approval.md
│   ├── journal.py             # Step 5: entry generation     → docs/journal-entries.md
│   ├── pipeline.py            # Orchestrator                 → docs/pipeline.md
│   └── prompts.py             # LLM prompt templates         → docs/llm-extraction.md
├── eval/
│   ├── __init__.py
...
# Set API key (required for LLM extraction and eval suite)
...
The eval suite (see `docs/eval-and-feedback.md`) IS the integration test. It runs the full pipeline end-to-end against labeled data.
...
| `docs/storage.md` | SQLite schema, seed data, invoice JSON encoding | `src/db.py`, `data/*.json` |
| `docs/po-matching.md` | PO lookup, tolerance validation, failure behavior | `src/po_matching.py` |
| `docs/llm-extraction.md` | LLM config, prompt design, post-extraction validation, batching, regulatory edge case | `src/attribute_extraction.py`, `src/prompts.py` |
...
| `docs/pipeline.md` | Orchestrator flow, mode semantics, transaction handling, error handling | `src/pipeline.py` |
...
| `docs/cli.md` | CLI commands, input interface, output format, demo sequence | `cli.py` |
